...
public class AsymStoreKey implements Serializable{
	...
	private static final Logger logger = LogManager.getLogger(AsymStoreKey.class);
	...
	public void assembleKey(String publicPartB64) throws InvalidKeySpecException, NoSuchAlgorithmException {
		publicKey = KeyFactory.getInstance("RSA")
				.generatePublic(new X509EncodedKeySpec(Base64.decodeBase64(publicPartB64)));
	...
	public byte[] decipher(byte[] bytes) throws Exception {
		try {
			...
			if (bytes!=null){ logger.debug("byte length at decipher" + bytes.length); }
