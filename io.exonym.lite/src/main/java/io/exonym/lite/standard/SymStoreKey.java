...
public class SymStoreKey implements Serializable{
	...
	private static final Logger logger = LogManager.getLogger(SymStoreKey.class);
	...
	public SymStoreKey() {
		...
		try {
			...
			logger.error("Mac Key Generation Error");
	...
	public static SymStoreKey build(String b64Encoded) throws Exception {
		if (b64Encoded!=null){
			...
			if (parts.length==2){
				byte[] sk = Base64.decodeBase64(parts[0].getBytes());
				byte[] mac = Base64.decodeBase64(parts[1].getBytes());
	...
	public String getBase64Encoded(){
		...
		String ss = Base64.encodeBase64String(s);
		String ms = Base64.encodeBase64String(m);
		return ss + ":" + ms;
	...
	public static SecretKey genSecretKey(){
		try {
			...
			logger.error("Exception on generation of SecrtetKey", e);
			return null;
	...
	public byte[] decipher(byte[] raw) throws Exception{	
		try {
			...
			logger.debug("Connection probably dropped at decipher.");
