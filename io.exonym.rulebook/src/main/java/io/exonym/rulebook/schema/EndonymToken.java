...
import io.exonym.rulebook.context.WalletUtils;
...
public class EndonymToken {
    ...
    private static final String ENDONYM_PREFIX = "urn:endonym:";
    private URI endonym;
    ...
    public static EndonymToken build(@Nullable URI endonym, PresentationToken pt) throws UxException {
        try {
    ...
    public static URI endonymForm(String scope, byte[] fullValue) throws UxException {
        ...
        URI result = URI.create(ENDONYM_PREFIX +
                prefix.substring(32) + ":" +
                n6 + "-" +
                shortValue);
        return result;
