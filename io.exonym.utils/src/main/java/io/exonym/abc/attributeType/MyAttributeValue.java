...
import io.exonym.abc.attributeEncoding.MyAttributeEncoding;
...
public abstract class MyAttributeValue {
  ...
  public MyAttributeValue(/*Nullable*/ EnumAllowedValues allowedValues) {
    if(allowedValues != null && allowedValues.getAllowedValues().size() > 0) {
      ...
      if(!isEnumeration()) {
        throw new RuntimeException("This type is not an enumeration, but EnumAllowedValues specified.");
      }
    } else {
      // Don't allow empty allowedValues
      this.allowedValues = null;
      if(isEnumeration()) {
        throw new RuntimeException("This type is an enumeration, but no EnumAllowedValues specified.");
      }
    }
  }
