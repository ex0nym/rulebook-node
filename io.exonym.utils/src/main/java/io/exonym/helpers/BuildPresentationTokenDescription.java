...
import io.exonym.actor.OwnedCredential;
import io.exonym.actor.PolicyReviewItem;
...
public class BuildPresentationTokenDescription {
	...
	private static final Logger logger = LogManager.getLogger(BuildPresentationTokenDescription.class);
			...
			for (CredentialInPolicy cip : p.getCredential()) {
				for (AttributeInPolicy aip : cip.getDisclosedAttribute()) {
					...
					if (ia!=null) {
						...
						if (ins.size() > 1) {
							logger.warn("Selecting only the first inspector in the list - choices of inspector has not been implemented.");
	...
	public HashMap<URI, HashSet<CandidateToken>>  setOwnedCredentials(ArrayList<OwnedCredential> ownedCredentials) throws Exception {
		...
		if (result.isEmpty()){
			...
			for (URI policyUid : result.keySet()){
				...
				logger.debug("empty=" + candidates.isEmpty() + " size=" + candidates.size());
	...
	private HashMap<URI, HashSet<CandidateToken>> collectCandidates()  throws UxException {
		...
		if (policyToCandidateMap.isEmpty()){
			throw new UxException("The Presentation Policy cannot be fulfilled");
	...
	private List<CredentialInToken> credentialInPolicyToToken(List<CredentialInPolicy> credentials,
															  HashMap<AttributeInPolicy, URI> inspectorChoiceUID) throws Exception {
		...
		for (CredentialInPolicy cred: credentials){
			...
			HashMap<URI, Attribute> attributes = ct.getOwnedCredential().getAttributes();
			...
			for (AttributeInPolicy aip : aips){
				...
				if (aip.getInspectorAlternatives()==null){
					Attribute attribute = attributes.get(aip.getAttributeType());
					ait.setAttributeValue(attribute.getAttributeValue());
	...
	private URI findMostRecentRevocationInformationUid(URI revocationAuthoirityUid) throws Exception {
		logger.debug("Skipping the building of MOST RECENT revocation information, " +
